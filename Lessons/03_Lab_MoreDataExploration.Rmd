...
# Matrices ----
matrix1 <- matrix(1:20, nrow = 5,ncol = 4) # default is assigning by columns
matrix1
matrix2 <- matrix(1:20, nrow = 5, ncol = 4, byrow = TRUE) # change default
matrix2
matrix3 <- matrix(1:20, nrow = 5, ncol = 4, byrow = TRUE, # return after comma continues the line
                  dimnames = list(c("uno", "dos", "tres", "cuatro", "cinco"), 
                                  c("un", "deux", "trois", "cat"))) # add column and row names
matrix3

matrix1[4, ] #row 4
matrix1[ , 3] # column 3
matrix1[c(12, 14)] # the 12th and 14th element
matrix1[c(12:14)] # the 12th to 14th element
matrix1[2:4, 1:3] # : all the way to
...
list1[[2]] #need to use double brackets for accessing list element
...
names(dataframe1) <- c("ID","Color","Passed"); # names the column for dataframe
View(dataframe1) 

dataframe1[1:2,] # the first two rows of the dataframe
dataframe1[c("ID","Passed")] # the ID and Passed columns
dataframe1$ID
...
```{r}
df_weather<-data.frame(high.temps.C, high.temps.F, low.temps.C, low.temps.F)
df_weather
colnames(df_weather)<-c("High Temperature(C)", "High Temperature(F)", "Low Temperature(C)", "Low Temperature(F)")
```
...
```{r}
summary(df_weather)
summary(df_weather$`High Temperature(C)` )
sd(df_weather$`High Temperature(C)`)
```
