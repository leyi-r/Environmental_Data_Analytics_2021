...
library(tidyverse)
library(lubridate)
#install.packages("trend") #run stationary test
library(trend)
#install.packages("zoo") #FOR missing observations in R
library(zoo)
#install.packages("Kendall") # for stationary test
library(Kendall)
...
```{r}
#Another way to write it: EnoDischarge.late_ts <- ts(EnoDischarge.late[[8]], start = c(1985,1), frequency = 365)
#define starting point, c(year, month), frequency
...
wind_data_clean <- 
  wind_data %>% 
  mutate( AWND.clean = zoo::na.approx(AWND) ) #zoo is the package, "::" is the way to call function from the package, na.approx is interpolation 
...
# look at the red line that makes up for the missing values
# if NA happens at the beginning or end of the set, na.omit or drop_na is okay; but if NA happens in the middle of a set, only use na.approx
```
...
#p-value is less than 0.05, reject null hypothesis that data is stationary. 

wind_data_trend2 <- trend::smk.test(wind_data_ts)
# Inspect results
wind_data_trend2
summary(wind_data_trend2)
# examine results for each season

#Visualization : use linear regression to show the trend
wind_data_plot <-
...
> Answer: the speed is decreasing in general
