...
# Fix this code (change the color outside of the aesthetics bracket)
...
# Filter dataset within plot building and facet by multiple variables (column by year, row by PM2.5)
PMplot.faceted2 <- 
...
# Depth in the fields of limnology and oceanography is on a reverse scale (scale_y_reverse)
tempvsdepth <- 
...
NvsP <-
  ...
  geom_abline(aes(slope = 16, intercept = 0)) #add reference line
print(NvsP)
...
Nutrientplot3 <-
  ...
  geom_boxplot(aes(color = nutrient)) # Why didn't we use "fill"?: color is changing the contour of the plot, color is changing the fill.
print(Nutrientplot3)
