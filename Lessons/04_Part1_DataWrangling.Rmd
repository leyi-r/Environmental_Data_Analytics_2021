...
# matrix filtering ( only filter rows so no things after comma, we are not filtering columns)
...
NTL.phys.data.PeterPaul3 <- filter(NTL.phys.data, lakename %in% c("Paul Lake", "Peter Lake")) #%in% percentage in percentage, it's like a matching operator; select lakename in all the names vector you are looking for
...
NTL.phys.data.JunethruOctober2 <- filter(NTL.phys.data, daynum > 151, daynum < 305) # , and & lead to the same thing
...
summary(NTL.phys.data.TuesdayLake$year4)
summary(NTL/NTL.phys.data$lakename, maxsum=3)
```
Question: Why don't we filter using row numbers?

> Answer: Because it will constantly change.
...
# date:1998-07-16, lakes: East Long Lake , depth:0.5
```
### Select
...
```{r}
# a cleaner way to do all the processes above
