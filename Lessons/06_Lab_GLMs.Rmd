...
# Evaluate assumption of normal distribution (use the sample to test)
shapiro.test((EPAair.subsample$Ozone)) #reject the normal hypothesis
ggplot(EPAair, aes(x = Ozone)) +
  geom_histogram() 
qqnorm(EPAair$Ozone); qqline(EPAair$Ozone)

#Run t-test (results: reject null)
...
#Compare variance using F-test (only); not bartlett test because only two groups
...
# Format as a t-test (reject null)
...
PM2.5byOzone.regression<_lm(data=EPAair, PM2.5~Ozone)
summary(PM2.5byOzone.regression) #ONLY 13% variation is explained by Ozone
...
OzonebyALL.regression<-lm(data=EPAair.subset, PM2.5 ~ Ozone + Year + Month + SITE_LONGITUDE +SITE_LATITUDE)
...
OzonebyBest.regression<-lm(data=EPAair.subset, PM2.5 ~ Ozone + Month + SITE_LONGITUDE +SITE_LATITUDE)
